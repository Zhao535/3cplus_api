...
public class TradeServiceImpl implements TradeService, TradeErrors {
    ...
    @PostConstruct
    public void init() {
        tradeCache = kvCacheFactory.create(new CacheOptions("trade", 1, 3600), new RepositoryProvider<Integer, Trade>() {
        ...
        jobManager.schedule("release_article", "0 0/30 * * * ?", new Job() {
            ...
            public void run() throws Exception {
                ...
                while (true) {
                    ...
                    items = items.stream().filter(it -> System.currentTimeMillis() > (it.getCreatedAt() + 30 * 60 * 1000)).collect(Collectors.toList());
                    for (Trade record : items) {
                        distributeLock.doInLock("release_article." + record.getId(), new ReleaseArticleWork(record), 5, 1800);
                    }
...
//        return null;
            ...
            for (ProductSpec spec : specs) {
                if (spec.getSno().equals(sno)) {
                    ...
                    if (CollectionUtils.isNotEmpty(secKillList)) {
                        secKillSet = secKillList.stream().filter(item -> item.getProductId().equals(product.getId())).collect(Collectors.toSet());
                    }
                            ...
                            for (TradePayload tradePayload : trade.getTradePayloads()) {
                                for (SecKill secKill : secKills) {
                                    if (tradePayload.getId().equals(secKill.getId())) {
                                        totalPrice += tradePayload.getRealPrice() * trade.getTradeItems().get(i).getNum();
                                    }
                                }
                            }
                        }
    ...
    public Integer couponAmount(Trade trade, Integer userCouponId) {
        ...
        for (TradeItem tradeItem : tradeItems) {
            ...
            if (payloadType == 1 || (payloadType == 2 && (product.getSequence().substring(0, 2).equals(payload.getCategory().getSequence().substring(0, 2)))) || (payloadType == 3 && (productId.equals(payload.getProduct().getId())))) {
                for (ProductSpec spec : specs) {
    ...
    public void wrapTrade(List<Trade> list, TradeWrapOption option) throws Exception {
        if (option.isWithMerchant()) {
            ...
            Map<Integer, Merchant> merchantMap = merchantService.findByIdIn(merchantIds).stream().collect(Collectors.toMap(Merchant::getId, it -> it));
        ...
        if (option.isWithUser()) {
            ...
            Map<Integer, User> userMap = userService.findAllById(userIds).stream().collect(Collectors.toMap(User::getId, it -> it));
            for (Trade trade : list) {
