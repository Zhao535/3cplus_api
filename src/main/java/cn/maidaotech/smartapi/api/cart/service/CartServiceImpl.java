...
public class CartServiceImpl implements CartService, CartErrors {
    ...
    public void init() {
        cartCache = kvCacheFactory.create(new CacheOptions("cart", 1, Constants.CACHE_REDIS_EXPIRE),
                new RepositoryProvider<Integer, Cart>() {
                    ...
                    @Override
                    public Cart findByKey(Integer id) throws
                            ServiceException {
                        return cartRepository.findById(id).orElse(null);
                    }

                    @Override
                    public Map<Integer, Cart> findByKeys(Collection<Integer> ids) throws
                            ServiceException {
    ...
    public Cart save(Cart cart) throws Exception {
        ...
        Integer userId = UserContexts.requestUserId();
        Cart exist = cartRepository.findByUserIdAndProductIdAndProductSno(userId, cart.getProductId(), cart.getProductSno());
    ...
    private void wrap(List<Cart> carts, CartWarpOption option) throws Exception {
        ...
        List<Integer> secKillIds = carts.stream().map(Cart::getCartPayload).filter(it -> it.getType() == PayloadType.SECKILL.value()).map(CartPayload::getId).collect(Collectors.toList());
        ...
        Map<Integer, Product> productMap = productService.findByIdIn(productIds).stream().collect(Collectors.toMap(Product::getId, item -> item));
        Map<Integer, SecKill> secKillMap = secKillService.findByIdIn(secKillIds).stream().collect(Collectors.toMap(SecKill::getId, item -> item));
